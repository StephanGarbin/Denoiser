...
{
	{
		...
		if (_firstTime)
		{
			...
			const int height = fr - fx;
			const int width = ft - fy;

			Denoise::Dimension imageDimension(width, height);

			//create an empty image data struct
			m_image = new Denoise::Image(imageDimension, Denoise::Image::FLOAT_4);
			...
			for (int ry = fy; ry < ft; ry++)
			{
				...
				foreach(z, readChannels)
				{
					...
					while (CUR < END)
					{
						m_image->setPixel(channel, ry, col, (float)*CUR);
			...
			Denoise::BM3DImageBlockProcessor processor(m_image, m_basic, m_result);
			//processor.process(m_settings, true);
	...
	foreach(z, channels) {
		...
		while (CUR < END) {
			...
			//++col;
			//continue;
			*CUR++ = m_result->getPixel(channel, y, col);
